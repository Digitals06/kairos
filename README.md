...
- **Scored-min in the weekly strip** — actual alive-time per run (`hit_count / avg_fps` summed over your week's local plays), labeled honestly, never guessed.
- **Weakest → strongest sorting** on every benchmark's Scenarios table — rank tier ascending, tiebroken by the score gap to the next tier, so closest-to-flip grinds head the list.
