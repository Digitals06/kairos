...
- **Grind chips on every family card** — unroll a family and each difficulty row carries a status chip: **▶ N runs to next tier** = the grind engine's count of scenarios that must improve to flip the rank (the cheapest single-scenario paths, or the plan moves when no single run can carry it; 0 means maxed), **plateaued** = mid-progress but no new PB in 3+ days, and **complete** when every scenario is at its top rung.
- **Scored-min in the weekly strip** — actual alive-time per run (`hit_count / avg_fps` summed over your week's local plays), labeled honestly, never guessed.
- **Weakest → strongest sorting** on every benchmark's Scenarios table — rank tier ascending, tiebroken by score gap to the next tier, so closest-to-flip grinds head the list.
...
- **Grind chips on family cards** — unroll and each difficulty row carries a status chip: **▶ N runs to next tier** = the grind engine's count of scenarios that must improve to flip the rank (the cheapest single-scenario paths, or the plan moves when no single run can carry it; 0 means maxed), **plateaued** = mid-progress but no new PB in 3+ days (max-rank difficulties never show this — they read complete), **complete** when every scenario is at its top rung.
- **Scored-min in the weekly strip** — actual alive-time per run (`hit_count / avg_fps` summed over your week's local plays), labeled honestly, never guessed.
- **Scenario sorting on deep pages** — the Scenarios table cycles **weakest → strongest → order**: weakest-first lifts the rank tier ascending and the closest-to-flip grinds head the list; strongest-first shows your maxed scenarios first.
